...
还有其他类型的建模器，例如隐式曲面创建系统（包括具有“团块状”外观的元球系统）[67, 558]，它们使用混合、权重和场等概念。这些建模器通过生成由某个函数方程 ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_01_0a05d480e57cac.png) 的解所定义的曲面，来创建有机形态。然后，使用移动立方体（marching cubes）等多边形化技术，生成用于显示的三角形集合（第17.3节）。
...
![图16.1 各种曲面细分方式](Real-Time_Rendering_4th_中文/assets/fig_16_2_16.1.png)
...
![图16.2 翘曲四边形及两种三角剖分](Real-Time_Rendering_4th_中文/assets/fig_16_2_16.2.png)
...
Schneider 和 Eberly [1574]、Held [714]、O’Rourke [1339] 以及 de Berg 等人 [135] 分别概述了多种三角剖分方法。最基本的三角剖分算法会检查多边形上任意给定两点之间的每一条线段，看它是否与多边形的任意一条边相交或重叠。如果是，就不能用这条线段分割多边形，于是继续检查下一对可能的点。否则，就用这条线段把多边形分成两部分，再以同样的方法对这些新多边形进行三角剖分。这种方法极其缓慢，复杂度为 O(![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_02_584fde46074e95.png))。


![图16.3 切耳法](Real-Time_Rendering_4th_中文/assets/fig_16_2_16.3.png)

**图 16.3** 切耳法。图中多边形在 ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_02_37d45ef245b1bd.png)、![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_02_25a7dc3c5b57aa.png) 和 ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_02_30543a5ab7b772.png) 处有潜在的耳。右图移除了 ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_02_25a7dc3c5b57aa.png) 处的耳。随后重新检查相邻顶点 ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_02_239e8d38e2bb18.png) 和 ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_02_30543a5ab7b772.png)，看它们现在是否构成耳；![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_02_30543a5ab7b772.png) 构成了耳。

更高效的方法是切耳法（ear clipping）；把它分成两个过程实施时，其复杂度为 O(![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_02_09c317452584b9.png))。首先遍历一次多边形以找出耳：考察所有顶点索引为 i、(i + 1)、(i + 2) 的三角形（索引对 n 取模），检查连接 i 与 (i + 2) 的线段是否不与任何多边形边相交。如果不相交，则 (i + 1) 处的三角形构成一个耳。见图 16.3。依次从多边形中移除每个可用的耳，并重新检查顶点 i 和 (i + 2) 处的三角形，看它们现在是否成为耳。最终所有耳都被移除，多边形也就完成了三角剖分。其他更复杂的三角剖分方法可以达到 O(n log n)，其中一些在典型情况下实际上可以达到 O(n)。Schneider 和 Eberly [1574] 给出了切耳法以及其他更快三角剖分方法的伪代码。
...
![图16.4 多轮廓转单轮廓](Real-Time_Rendering_4th_中文/assets/fig_16_2_16.4.png)
...
![图16.5 模板缓冲奇偶填充](Real-Time_Rendering_4th_中文/assets/fig_16_2_16.5.png)
...
![图16.6 对角线选择与着色差异](Real-Time_Rendering_4th_中文/assets/fig_16_2_16.6.png)
...
![图16.7 扭曲四边形纹理映射](Real-Time_Rendering_4th_中文/assets/fig_16_2_16.7.png)
...
![图16.8 边缘裂缝与缝合](Real-Time_Rendering_4th_中文/assets/fig_16_2_16.8.png)
...
![图16.9 T形顶点造成的着色不连续](Real-Time_Rendering_4th_中文/assets/fig_16_2_16.9.png)
...
![图 16.10：根据共享边的遍历顺序统一多边形朝向](Real-Time_Rendering_4th_中文/assets/fig_16_3_16.10.png)
...
![图 16.11：没有逐顶点法线与具有逐顶点法线的对象](Real-Time_Rendering_4th_中文/assets/fig_16_3_16.11.png)
...
![图 16.12：法线平均与按顶点角加权的比较](Real-Time_Rendering_4th_中文/assets/fig_16_3_16.12.png)
...
对于高度场，Shankel [1614] 展示了如何通过求各坐标轴方向上邻点的高度差，快速近似按角度加权方法得到的平滑结果。给定一个点 **p** 和四个相邻点：高度场 x 轴方向上的 ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_03_48ceb6deba93d3.png)、![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_03_719058da8efa3c.png)，以及 y 轴方向上的 ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_03_dc55d2590aab74.png)、![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_03_f2bf45adb780d3.png)，**p** 处的（未归一化）法线可以较好地近似为


![数学公式](Real-Time_Rendering_4th_中文/assets/math/eq_16_03_c552684f626288.png)
...
![图16.13 三角形扇与凸多边形](Real-Time_Rendering_4th_中文/assets/fig_16_4_16.13.png)

图 16.13：左图说明三角形扇的概念。三角形 ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_4b1d7908f5e266.png) 发送顶点 ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_08896f686baac3.png)（中心顶点）、![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_5607dc78935cb8.png) 和 ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_37d45ef245b1bd.png)。后续三角形 ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_130f49865f8afc.png)（i > 0）只发送顶点 ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_146f26fb6d871d.png)。右图为凸多边形，它总能转换成一个三角形扇。

由 n 个顶点组成的三角形扇定义为一个有序顶点列表：


![数学公式](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_80ba40f085776f.png)


其中 ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_08896f686baac3.png) 是中心顶点；在该列表上规定一种结构，使第 i 个三角形为


![数学公式](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_4929a99be8de11.png)


其中 0 ≤ i < n − 2。

如果三角形扇包含 m 个三角形，那么第一个三角形要发送三个顶点，剩余 m − 1 个三角形每个再发送一个顶点。这意味着，对于长度为 m 的顺序三角形扇，平均每个三角形发送的顶点数 ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_1840d2964599e4.png) 可表示为


![数学公式](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_355f1fe750d93f.png)


很容易看出，当 m → ∞ 时，![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_1840d2964599e4.png) → 1。这看起来或许与实际情况关系不大，但可以考虑一个更现实的数值。如果 m = 5，那么 ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_1840d2964599e4.png) = 1.4，也就是说，平均每个三角形只发送 1.4 个顶点。

### 16.4.2 三角形带

三角形带与三角形扇相似，都会重用先前三角形中的顶点。不过，它重用的不是一个固定的中心点和前一个顶点，而是前一个三角形的两个顶点，用它们帮助构成下一个三角形。考虑图 16.14。如果把这些三角形当作一个带，就能以更紧凑的方式将它们发送到渲染流水线。对于第一个三角形（记为 ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_4b1d7908f5e266.png)），依次发送全部三个顶点（记为 ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_08896f686baac3.png)、![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_5607dc78935cb8.png)、![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_37d45ef245b1bd.png)）。对于带中的后续三角形，只须发送一个顶点，因为另外两个顶点已经随前一个三角形发送过。例如，发送三角形 ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_47714cb824af95.png) 时，只发送顶点 ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_239e8d38e2bb18.png)，并利用三角形 ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_4b1d7908f5e266.png) 的顶点 ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_5607dc78935cb8.png) 和 ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_37d45ef245b1bd.png) 来构成 ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_47714cb824af95.png)。对于三角形 ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_85e3550cabd641.png)，只发送顶点 ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_25a7dc3c5b57aa.png)，带中的其余部分依此类推。


![图16.14 三角形带](Real-Time_Rendering_4th_中文/assets/fig_16_4_16.14.png)
...
![数学公式](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_74d74a6e49eab0.png)


在该列表上规定一种结构，使第 i 个三角形为


![数学公式](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_d1fd3b3d727b57.png)


其中 0 ≤ i < n − 2。这种带称为顺序三角形带，是因为顶点按给定次序发送。这一定义意味着，具有 n 个顶点的顺序三角形带包含 n − 2 个三角形。

对于长度为 m（即包含 m 个三角形）的三角形带，其平均顶点数也记为 ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_1840d2964599e4.png)，分析与三角形扇相同（见式 16.4），因为二者的启动阶段相同，之后每个新三角形都只发送一个顶点。同样，当 m → ∞ 时，三角形带的 ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_1840d2964599e4.png) 自然也趋于每个三角形一个顶点。当 m = 20 时，![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_1840d2964599e4.png) = 1.1，远好于 3，并且已接近极限值 1.0。与三角形扇一样，第一个三角形总要付出三个顶点的启动开销，这一开销由后续三角形分摊。
...
![数学公式](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_02313e0ee2f88a.png)


这里，v 是顶点数，e 是边数，f 是面数，g 是亏格。亏格是物体中孔洞的数量。例如，球面的亏格为 0，环面的亏格为 1。这里假定每个面只有一个边界环。如果面可以有多个边界环，公式变为


![数学公式](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_1eed0314cdf870.png)
...
总的来说，离线预处理三角形网格能够明显改善顶点缓存性能；当顶点阶段是瓶颈时，还能提高总体帧率。这种处理很快，在实践中实际达到 ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_3657599716cc3d.png)。已有若干开源版本可用 [485]。由于这类算法可以自动应用于网格，优化不增加存储开销，也不影响工具链中的其他工具，因此这些方法通常是成熟开发系统的一部分。例如，Forsyth 算法似乎就是 PLAYSTATION 网格处理工具链的一部分。虽然现代 GPU 采用统一着色器架构后，顶点变换后缓存已经发生演变，但避免缓存未命中仍是重要问题 [530]。

### 16.4.5 顶点与索引缓冲区／数组

向现代图形加速器提供模型数据的一种方式，是使用 DirectX 所称的顶点缓冲区，以及 OpenGL 所称的顶点缓冲区对象（VBO）。本节采用 DirectX 术语，所介绍的概念在 OpenGL 中都有对应形式。

顶点缓冲区的思想，是在一块连续内存中保存模型数据。顶点缓冲区是采用特定格式的顶点数据数组。格式规定顶点是否包含法线、纹理坐标、颜色或其他特定信息。每个顶点的数据集中成组，各顶点依次排列。一个顶点占用的字节数称为其步长。这种存储方式称为交错缓冲区。另一种方式是使用一组顶点流。例如，一个流保存位置数组 {![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_c11de44d831b47.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_19aa1dc33bda1e.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_3a55303a18d915.png) …}，另一个流单独保存法线数组 {![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_8794b6a3c050bf.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_3a00723a6f5471.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_d13080baeee9f5.png) …}。实践中，将每个顶点的全部数据放在单一缓冲区中，通常在 GPU 上更高效，但优势还没有大到必须避免多流的程度 [66, 1494]。多流的主要开销是额外的 API 调用；当应用程序受 CPU 限制时，这一开销可能值得避免，否则并不显著 [443]。
...
![图16.15 顶点和索引缓冲区的组织方式](Real-Time_Rendering_4th_中文/assets/fig_16_4_16.15.png)

图 16.15：定义图元的不同方式，从上至下大致按内存使用量由多到少排列：独立三角形、顶点三角形列表、使用两个或一个数据流的三角形带，以及列出独立三角形或按三角形带顺序排列的索引缓冲区。

图内文字与数据完整译录：

- 三个三角形，由顶点位置 ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_c11de44d831b47.png) 至 ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_6aa5478e61d94d.png)、法线 ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_8794b6a3c050bf.png) 至 ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_f2a16a55b07b30.png) 组成。
- 可以通过一系列独立调用渲染这些三角形：begin, ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_c11de44d831b47.png), ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_8794b6a3c050bf.png), ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_19aa1dc33bda1e.png), ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_3a00723a6f5471.png), ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_3a55303a18d915.png), ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_d13080baeee9f5.png), end, begin, ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_19aa1dc33bda1e.png), ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_3a00723a6f5471.png), ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_6aa5478e61d94d.png), ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_f2a16a55b07b30.png), ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_3a55303a18d915.png), ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_d13080baeee9f5.png), end, begin, ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_3a55303a18d915.png), ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_d13080baeee9f5.png), ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_6aa5478e61d94d.png), ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_f2a16a55b07b30.png), ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_c11de44d831b47.png), ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_8794b6a3c050bf.png), end。
- 位置和法线可以放入两个独立列表。将这两个数组视为三角形列表，使数组中每个互不重叠的三元组构成一个三角形。位置数组为 [![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_c11de44d831b47.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_19aa1dc33bda1e.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_3a55303a18d915.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_19aa1dc33bda1e.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_6aa5478e61d94d.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_3a55303a18d915.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_3a55303a18d915.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_6aa5478e61d94d.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_c11de44d831b47.png)]；法线数组为 [![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_8794b6a3c050bf.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_3a00723a6f5471.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_d13080baeee9f5.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_3a00723a6f5471.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_f2a16a55b07b30.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_d13080baeee9f5.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_d13080baeee9f5.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_f2a16a55b07b30.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_8794b6a3c050bf.png)]。
- 位置和法线也可以放入数组，让每个连续三元组定义一个三角形。位置数组为 [![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_c11de44d831b47.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_19aa1dc33bda1e.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_3a55303a18d915.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_6aa5478e61d94d.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_c11de44d831b47.png)]；法线数组为 [![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_8794b6a3c050bf.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_3a00723a6f5471.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_d13080baeee9f5.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_f2a16a55b07b30.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_8794b6a3c050bf.png)]。
- 每个顶点可以放入一个交错数组，使每个互不重叠的三元组，或每个连续三元组（即三角形带）构成一个三角形。图示三角形带的顶点数组为 [![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_c11de44d831b47.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_8794b6a3c050bf.png)　![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_19aa1dc33bda1e.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_3a00723a6f5471.png)　![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_3a55303a18d915.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_d13080baeee9f5.png)　![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_6aa5478e61d94d.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_f2a16a55b07b30.png)　![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_c11de44d831b47.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_8794b6a3c050bf.png)]。
- 每个顶点可以放入单一数组，并使用索引列表指定独立三角形。顶点数组为 [![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_c11de44d831b47.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_8794b6a3c050bf.png)　![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_19aa1dc33bda1e.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_3a00723a6f5471.png)　![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_3a55303a18d915.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_d13080baeee9f5.png)　![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_6aa5478e61d94d.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_f2a16a55b07b30.png)]；索引数组为 [0 1 2　1 3 2　2 3 0]。
- 每个顶点可以放入单一数组，并使用索引列表定义三角形带。顶点数组为 [![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_c11de44d831b47.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_8794b6a3c050bf.png)　![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_19aa1dc33bda1e.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_3a00723a6f5471.png)　![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_3a55303a18d915.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_d13080baeee9f5.png)　![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_6aa5478e61d94d.png) ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_04_f2a16a55b07b30.png)]；索引数组为 [0 1 2 3 0]。
...
![图16.16 火山口湖高度场及简化网格](Real-Time_Rendering_4th_中文/assets/fig_16_5_16.16.png)
...
![图16.17 边折叠操作](Real-Time_Rendering_4th_中文/assets/fig_16_5_16.17.png)
...
![图16.18 不良折叠造成边交叉](Real-Time_Rendering_4th_中文/assets/fig_16_5_16.18.png)
...
![数学公式](Real-Time_Rendering_4th_中文/assets/math/eq_16_05_88728472822670.png)


是新位置 **v** 相对于 m 个平面的代价函数，其中 ![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_05_34778776540256.png) 是平面 i 的法线，![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_05_8039a811d0b4aa.png) 是该平面相对于原点的偏移量。
...
![图16.19 同一条边的两种收缩](Real-Time_Rendering_4th_中文/assets/fig_16_5_16.19.png)
...
![图16.20 飞机网格简化](Real-Time_Rendering_4th_中文/assets/fig_16_5_16.20.png)

**图 16.20** 网格简化。左上图为包含 13,546 个面的原始网格，右上图简化为 1,000 个面，左下图简化为 500 个面，右下图简化为 150 个面 [770]。（图片 ©1996 Microsoft。保留所有权利。）


![图16.21 保持纹理的网格简化](Real-Time_Rendering_4th_中文/assets/fig_16_5_16.21.png)
...
![图16.22 自动简化的对称性问题](Real-Time_Rendering_4th_中文/assets/fig_16_5_16.22.png)
...
索引缓冲区的格式中就存在一种简单的压缩方式。索引缓冲区由无符号整数数组组成，这些整数给出顶点在顶点缓冲区数组中的位置。如果顶点缓冲区中的顶点数小于或等于![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_06_2beea91ec90c47.png)，那么索引缓冲区就可以使用无符号短整数，而不是无符号长整数。某些API支持对顶点数少于![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_06_c04da3533f52b4.png)的网格使用无符号字节，但这样可能造成代价高昂的对齐问题，因此通常会避免使用。值得注意的是，OpenGL ES 2.0、未启用扩展的WebGL 1.0，以及某些较老的台式机和笔记本电脑GPU，都存在不支持无符号长整数索引缓冲区的限制，因此必须使用无符号短整数。
...
即使存储温度本身，也可能只需要精确到小数点后几位。一个浮点数的总精度为24位，略多于7位十进制有效数字。请注意，16位就能提供接近5位十进制有效数字的精度。温度值的范围很可能足够小，以至于不需要浮点格式的指数部分。以最小值作为偏移量，以最大值减去最小值作为缩放量，就能把这些数值均匀分布到一个有限的范围内。例如，如果数值范围是28.51至197.12，那么可以把一个无符号短整数值转换为温度：先将它除以![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_06_81e2c04de949a7.png)，再将结果乘以缩放因子（197.12 − 28.51），最后加上偏移量28.51。只要存储数据集的缩放与偏移因子，并将它们传给顶点着色器程序，数据集本身就只需原来一半的存储空间。这种变换称为**标量量化** [1099]。


![图16.23 顶点数据的固定码率压缩](Real-Time_Rendering_4th_中文/assets/fig_16_6_16.23.png)
...
其他顶点数据也可能有专门适用的压缩技术。纹理坐标通常限制在[0.0, 1.0]范围内，因此一般可以安全地缩减为无符号短整数，隐含的偏移量为0，缩放除数为![数学符号](Real-Time_Rendering_4th_中文/assets/math/eq_16_06_81e2c04de949a7.png)。纹理坐标通常成对出现，恰好可以存进两个无符号短整数 [1381]；根据精度要求，甚至只需要3字节 [88]。
